...
tBrakeCaliper = movmean(Data.tBrakeFR(:,2),100);
pBrakeF = movmean(Data.pBrakeF(:,2),100);
...
subplot(3, 1, 3);
plot(Timestamp(:,2), TempC(:,2), 'r', 'LineWidth', 2);
xlabel('Time');
...
T(1:maxDelay) = tBrakeCaliper(maxDelay);
...
[x, err] = surrogateopt(f,[0 0 0.05 0.5],[50 1 0.15 1.5],[],[],[],[],[], options);   % Funzione che da in output x e err

[err,Torque,P_tot,P_mech,P_conv,P_irr,T,t] = optFUN(x,mu,BPS,nP,dM,dP,wWheel,caliperArea,caliperMass,Cp,T,T0,TempC, fine, Timestamp,maxDelay);    % Ricalcolo con valori ottimizzati
...
% cp_TOT = ((Cp(floor(T(t-1)))*caliperMass) + 0.050*500*2 + 0.010*540*nP)/(caliperMass+0.05*2+0.010*nP);




% Calcolo temperatura secondo modello analitico
function [err,Torque,P_tot,P_mech,P_conv,P_irr,T,t] = optFUN(x,mu,BPS,nP,dM,dP,wWheel,caliperArea,caliperMass,Cp,T,T0,TempC, fine,Timestamp,maxDelay)
...
% delay = round(x(4));  % Arrotonda delay al valore intero più vicino
...
for t=(maxDelay+1):fine

    cp_TOT = ((Cp(floor(T(t-1)))*caliperMass) + 0.050*500*2 + 0.010*540*nP);
    ...
    deltaT(t)   = (P_tot(t).*(Timestamp(t,2)-Timestamp(t-1,2)))./(cp_TOT*x(4));   % Incremento di temperatura
    
    T(t)        = T(t-1)+deltaT(t);
end
err = rms(T(maxDelay:fine)'-TempC(maxDelay:fine,2));
